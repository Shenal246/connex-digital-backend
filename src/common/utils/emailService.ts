...
export const emailService = {
    async sendEmail(to: string, subject: string, text: string, html?: string) {
        if (!env.SMTP_USER || !env.SMTP_PASS) {
            ...
            logger.warn(`[MOCK EMAIL BODY]\n${text}`);
            return;
        }

        try {
            const info = await transporter.sendMail({
                from: env.SMTP_FROM || `"HR Platform" <${env.SMTP_USER}>`,
                to,
                subject,
                text,
                html,
            });
            logger.info(`[EMAIL_SERVICE] Email sent to ${to} (ID: ${info.messageId})`);
        } catch (error) {
            logger.error(error, `[EMAIL_SERVICE] Failed to send email to ${to}`);
        }
    },
    ...
    async sendWelcomeEmail(to: string, name: string, tempPassword: string) {
        ...
        await this.sendEmail(
            to,
            'Your Connex Digital Platform Credentials',
            `Welcome ${name},\n\nYour account has been created.\n\nUsername: ${to}\nTemporary Password: ${tempPassword}\n\nPlease login at ${loginUrl} to change it.`,
            html
        );
    },
    ...
    async sendPasswordResetEmail(to: string, resetLink: string) {
        await this.sendEmail(
            ...
            `<p>Click the link below to reset your password:</p><p><a href="${resetLink}">${resetLink}</a></p><p>This link will expire in 15 minutes.</p>`
        );
    }
};
